...
cdef class EdjeEdit(Edje): # [object PyEdjeEdit, type PyEdjeEdit_Type]:
    ...
    def part_add(self, char *name, int type, char *source=""):
        if type != EDJE_PART_TYPE_EXTERNAL:
            return bool(edje_edit_part_add(
                self.obj, name, <Edje_Part_Type>type))
        else:
    ...
    property error:
        def __get__(self):
            last_error = eina_error_get()
            if last_error:
               return eina_error_msg_get(last_error)
