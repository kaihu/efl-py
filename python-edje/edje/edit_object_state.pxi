...
cdef class State:
    ...
    def external_params_get(self):
        ...
        lst = edje_edit_state_external_params_list_get(self.edje.obj, self.part,
                                                       self.name, self.value)
        while lst:
            p = ExternalParam_from_ptr(<Edje_External_Param *>lst.data)
            if p is not None:
                ret.append(p)
            lst = lst.next
        return ret
    ...
    def external_param_get(self, param):
        cdef Edje_External_Param_Type type
        cdef void *value
        ...
        if not edje_edit_state_external_param_get(self.edje.obj, self.part,
                                                  ...
                                                  &type, &value):
            return None
        if type == EDJE_EXTERNAL_PARAM_TYPE_BOOL:
            b = (<Eina_Bool *>value)[0]
            return (type, bool(b))
        elif type == EDJE_EXTERNAL_PARAM_TYPE_INT:
            i = (<int *>value)[0]
            return (type, i)
        elif type == EDJE_EXTERNAL_PARAM_TYPE_DOUBLE:
            d = (<double *>value)[0]
            return (type, d)
        elif type == EDJE_EXTERNAL_PARAM_TYPE_STRING:
            s = <char *>value
            if s == NULL:
                return (type, None)
            else:
                return (type, s)
        elif type == EDJE_EXTERNAL_PARAM_TYPE_CHOICE:
            s = <char *>value
            if s == NULL:
                return (type, None)
            else:
    ...
    def external_param_set(self, param, value):
        ...
        if isinstance(value, bool):
            ...
            if t == EDJE_EXTERNAL_PARAM_TYPE_STRING:
